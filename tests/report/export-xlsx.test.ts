...
import { GET } from "@/app/api/report/export/visit-daily.xlsx/route";
...
describe("PROMPT 9 — validation export-xlsx", () => {
  it("returns 422 if start_date missing", async () => {
    const req = new Request(
      "http://localhost/api/report/export/visit-daily.xlsx?end_date=2026-01-31"
    );
  ...
  it("returns 422 if end_date missing", async () => {
    const req = new Request(
      "http://localhost/api/report/export/visit-daily.xlsx?start_date=2026-01-01"
    );
    const res = await GET(req);
    expect(res.status).toBe(422);
