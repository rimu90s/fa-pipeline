...
export async function GET(req: Request) {
  const endpoint = "/api/report/export/weekly-leads-summary.csv";
  ...
  try {
    ...
    if (!start_date || !end_date) {
      return jsonErr(request_id, 422, "BAD_REQUEST", "start_date & end_date required");
    }
    ...
    await writeAuditLog({
      action: "REPORT_EXPORT_WEEKLY_LEADS_SUMMARY",
      companyId: ctx.companyId,
      branchId: null,
