...
export async function GET(req: Request) {
  try {
    ...
    const rows: DailyLeadsSummaryRow[] = await selectDailyLeadsSummaryFromView({
    ...
    await writeAuditLog({
      action: "REPORT_EXPORT_DAILY_LEADS_SUMMARY",
      companyId: ctx.companyId,
      branchId: null,
    ...
    // pilih kolom yang stabil (sisanya boleh kamu tambah sesuai VIEW)
    const cols = ["date", "unit_kerja_id"];
    ...
    return new NextResponse(csv, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${fileName("daily-leads-summary", start_date, end_date)}"`,
      },
    });
  } catch (e: unknown) {
